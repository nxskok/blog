...
I didn't get an answer to Maelle's question of why `haven` worked less well for me than `rio` a long time ago. After all, `rio` *uses* `haven` when `haven` will work. 
...
I had to allow for five separate columns because a respondent might have used all the different ways of getting to work this month, but there are a lot of missings because most people used only one or two methods. Undaunted, we `gather` up the response columns:
...
I don't know why this gives me a warning, but it seems to work.
...
I think my days of using `stringr::str_split` in situations like this are over. 
...
This web site contains many tables for the English Premier soccer league, for example the current standings. I have to supply a `format` because the URL does not end in `.html`:
