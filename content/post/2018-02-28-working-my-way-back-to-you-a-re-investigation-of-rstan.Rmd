...
Stan runs either independently (via the command-line version of Stan called `cmdstan`) or connected with other languages such as R, Python, [and others](http://mc-stan.org/users/interfaces/index.html).  We will use the R interface, which works via the package `rstan`. I probably need the `tidyverse` as well:
