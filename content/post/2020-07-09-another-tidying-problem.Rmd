...
```{r}
gas <- read_table("gas-mileage.txt", col_names = FALSE)
gas
```
...
The things in `X1` are our column-names-to-be, and the values that go with them are in `var_value`. `var_name` has served its purpose; these are the original columns in the data file, which we don't need any more. So now, we make this wider, right?
...
This has worked. The fillup numbers have come out in the wrong order, but that's probably not a problem. It would also work if you had a different number of observations on each row of the original data file, as long as you had a fillup number, a computer value and a driver value for each one.
