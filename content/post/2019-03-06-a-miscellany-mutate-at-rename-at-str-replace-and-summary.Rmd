...
and then, up, look up those jobs. I carefully made the numerical jobs be called `job` in both places, so it's the easy form of `left_join`. `r tufte::margin_note("I have never used *right_join*. I think it would confuse me too much.")`
...
We have the *values* short enough to look at, but the column *names* are still too long. How can we shorten them?

The columns with names like `posterior.custserv` ought to have names like `p.custserv`. So what we are aiming for is to look for all column names that start with `posterior` and text-replace the `posterior` with `p`. This uses the `stringr` function `str_replace` which requires three inputs: a string, a piece of text to look for, and another piece of text to replace it with. Let's keep the rounding in as well:

```{r}
d %>% 
  mutate_at(vars(starts_with("posterior")), ~round(., 3)) %>%
  rename_at(vars(starts_with("posterior")), ~str_replace(., "posterior", "p")) %>% 
  head()
```

This is a much more pleasant display. We can see that the first individual is almost certainly in customer service, the second individual could be in customer service or a mechanic, and so on.
...
- LD2 depends mostly (and positively) on `outdoor`.
...
Oh yes, that shows how many people of each job we have, which is much more helpful.

The `mutate_if` says "for each column that is text (have the property of being `character`), run `factor` on it", temporarily creating a factor for the purpose of passing it into `summary`.
...
Their scores were 10 on `outdoor`, 22 on `social` and 5 on `conservative`. According to the summary, this is low on `outdoor`, above the median on `social` and low on `conservative`. High social and low conservative makes this person high on LD1, and low outdoor makes this person *high* on LD2.
...
High LD1 and high LD2 puts this person squarely in with the red dots that represent customer service. With this kind of profile, it is unlikely that person #1 does any other kind of job. There is a little overlap between the colours, but not much.