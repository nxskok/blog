...
![](/Screenshot_2019-12-01_17-44-05.png)
...
`r tufte::margin_note("The data file is on the github repo somewhere. I have to organize things so you can get at it.")`

```{r}
my_file <- "/home/ken/teaching/blog/detergent.csv"
...
So now it looks as if I want to `pivot_wider` that column `what`, getting the values from `value`. (At this point, I feel a nagging doubt that I can do this with one `pivot_longer`, but anyway):
`r tufte::margin_note("I don't actually think I can here. I was thinking of .value, but that is used when the names of the columns that I'm making longer contain the names of new columns in them.")`
...
Statistically, if the two traces are more or less parallel, the two factors detergent and temperature act independently on the amount of dirt removed. But that is not the case here: a warm temperature is the best for Best detergent, while a hot temperature is best for Super detergent. 
`r tufte::margin_note("There are always two ways to express an interaction effect. The other one here is that the two detergents are pretty similar except at warm water temperatures, where Best is a lot better.")`
...
```{r}
my_file="/home/ken/teaching/blog/laundry.txt"
...
My first go at this treated the comma as a thousands separator (which was then dropped), so the top left cell got read as the number 4565. This use of `col_types` forces the columns to be character. 
...
We have several values for dirt removed, separated by columns. We could use `separate` to create four new columns and pivot *them* longer as well. But there is a better way:
...
- the default separator between values is everything non-alphanumeric except for a dot, which is fine here (comma is one of those)
