...
[this site](http://blog.fellstat.com/?cat=15), this is a "special Mercator projection", and the way to put longitude and latitude on the axes is this:
...
`openproj` will accept a variety of projections according to the `proj4` standard [here](http://proj4.org/). I need the right one that will display longitude and latitude on a good scale, but I don't know which one to use. I welcome any feedback on that, in the comments below. Or tips on how to convert latitude and longitude into this scale, which would actually be better. 
...
For labelling points on a `ggplot` I like `geom_text_repel` from package `ggrepel`. This labels points in such a way that they don't overplot each other