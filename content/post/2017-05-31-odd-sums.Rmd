...
A second insight is that we can exchange any rows or any columns without affecting the row and column sums. So that means ("without loss of generality" as the mathematicians say) that we only need to consider arrays with a particular number, say 1, at the top left corner. Since there are only five odd numbers, if we have 1 top left, the other odd numbers 3, 5, 7 and 9 must also be in the first row and first column, with the four even numbers at the bottom right.
...
This is about five times smaller than my simulation gave. I think there is an error in my logic rather than my simulation, but I can't find it (yet). Bum.
