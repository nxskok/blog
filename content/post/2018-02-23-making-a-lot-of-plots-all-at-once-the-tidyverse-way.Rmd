...
This says, in words, "for each value in the `lambda` column, run the function `random_poisson` with that `lambda` value as its first input and the fixed value 100 as its second, storing the results in `randoms`". 
...
That thing `randoms` is a so-called **list-column**. In a list-column, each entry can be any kind of thing that you can save in an R variable, in this case a vector of 100 integers. Except that you can't see the actual values yet.
...
Now, the `map`. This requires a bit of extra care, since the "for each" part has to loop over *two* variables: the random sample and the value of `lambda`. This requires using `map2`, which goes like this:
